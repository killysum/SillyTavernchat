...
- **一键删除60天内未登录用户全部数据**：管理员可一键删除60天内未登录用户全部数据，包括用户数据、备份文件、聊天记录等。由于酒馆玩法单一，有些用户在使用一段时间就没有任何玩下去的兴趣。但是储存日益庞大。经过测试60天未登录用户，基本上都是没有玩下去的兴趣，所以这个删除60天内未登录用户对用户体验并不会造成影响。  2025/11/07新增
